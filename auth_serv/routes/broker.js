...
router.get("/", async (req, res) => {
    try {
        var brokers = await mqttbaru.methods.getBroker().call({ from: accounts[5] });
        res.send(brokers)
    } catch (error) {
...
router.post("/add", async (req, res) => {
    try {
        ...
        var transaction = await mqttbaru.methods.addBroker(address).send({ from: accounts[5] })
...
router.post("/delete", async (req, res) => {
    try {
        const { address } = req.body;
        var transaction = await mqttbaru.methods.deleteBroker(address).send({ from: accounts[5] })
        res.send(transaction)
    } catch (error) {
